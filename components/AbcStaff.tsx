...
function buildNoteXMap(
  ...
  const noteEls = Array.from(container.querySelectorAll<SVGElement>(".abcjs-note"))
...
function interpolateX(map: NoteXMap, currentTick: number, totalTicks: number, containerWidth: number): number | null {
...
export default function AbcStaff({ score, title, currentTime, tempo, noteResults }: Props) {
  ...
  const rebuildNoteXMap = useCallback(() => {
    ...
    noteXMapRef.current = buildNoteXMap(containerRef.current, tickOffsets, score)
  }, [score])
  ...
  const renderAbc = useCallback(() => {
    ...
    const barsPerLine = Math.max(1, Math.floor(containerWidth / PIXELS_PER_BAR))

    const abcString = scoreToAbc(score, title, tempo, barsPerLine)
    ...
    import("abcjs").then((abcjs) => {
      ...
      // abc.js sets its own width — scale the SVG to fill our container
      const svg = containerRef.current.querySelector("svg")
      if (svg) {
        const svgWidth = parseFloat(svg.getAttribute("width") ?? "0")
        if (svgWidth > 0) {
          // Convert fixed width/height to viewBox so CSS scaling works
  ...
  }, [score, title, tempo, rebuildNoteXMap])
  ...
  }, [currentTime > 0, startPlayhead, stopPlayhead])
  ...
  return (
    ...
    <div ref={wrapperRef} style={{ maxWidth: MAX_CONTAINER_WIDTH }} className="relative w-full mx-0 my-auto">
      {/* containerRef: abc.js renders into here */}
      <div ref={containerRef} id="abcjs-container" />
