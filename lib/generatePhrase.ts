...
export const TONIC_TO_NUMBER: Record<KeyTonic, number> = {
  C: 0,
  Db: 1,
  "C#": 1,
  D: 2,
...
const KEYS_BY_DIFFICULTY: Record<number, KeyTonic[]> = {
  1: ["C"],
  2: ["C", "G", "F", "Bb"],
  3: ["C", "G", "F", "Bb", "D", "Eb", "A"],
  4: ["C", "G", "F", "Bb", "D", "Eb", "A", "E", "Ab", "B"],
  5: [
    "C",
    "G",
    "F",
...
// function deriveKey(tonicMidi: number): KeySig {
//   const pc = ((tonicMidi % 12) + 12) % 12;
//   return { tonic: PC_TO_KEY_ROOT[pc], mode: "major" };
// }

function randomKey(difficulty: number, rng: RNG): KeySig {
  const keys = KEYS_BY_DIFFICULTY[difficulty];
  if (!keys) throw new Error(`bro's cooked`);
  const tonic = keys[Math.floor(rng() * keys.length)];
...
function getDifficultyBand(difficulty: number): DifficultyBand {
  const minTarget =
    DIFFICULTY_TARGETS[difficulty as 1 | 2 | 3 | 4 | 5] ??
    DIFFICULTY_TARGETS[1];
  const maxTarget =
...
function generatePhraseOnce(settings: GenerationSettings = {}): {
  ...
  const key: KeySig = randomKey(difficulty, rng);

  const tonic: number = TONIC_TO_NUMBER[key.tonic]; // 0 - 11 for each of the 12 notes

  if (!settings.instrument) throw new Error("pls instrument (bro's cooked)");
  ...
  const minMidi = Math.max(range.minMidi, tonicMidi - spread.below);
  const maxMidi = Math.min(range.maxMidi, tonicMidi + spread.above);
  ...
  for (let m = 0; m < bars; m++) {
    ...
    while (used < MEASURE_TICKS) {
      ...
      const allCandidates = BASE_CELLS.filter(
        (c) =>
          totalTicks(c.durs) <= remaining &&
          (c.minDifficulty ?? 1) <= difficulty,
      );
      ...
      const perCellCeiling =
        (budgetLeft / Math.max(estimatedCellsRemaining, 1)) * 1.4;

      // Prefer cells within budget; fall back to all if nothing qualifies
      const budgeted = allCandidates.filter(
        (c) => cellDifficulty(c) <= perCellCeiling,
      );
    ...
    for (const cell of upgraded[m]) {
      ...
      for (let i = 0; i < cell.durs.length; i++) {
        ...
        if (isRest) {
          ...
          const stepsUp =
            ((relStep % SCALE.length) + SCALE.length) % SCALE.length;
          const octaves = Math.floor(relStep / SCALE.length);
          const increment = SCALE[stepsUp] + octaves * 12;

          const midiNote = Math.min(
            maxMidi,
            Math.max(minMidi, tonicMidi + increment),
          );
  ...
  if (diffRange === 0 || isNaN(diffRange)) {
    console.warn(
      `[difficulty] Range is ${diffRange}, using overallDifficulty directly`,
    );
    adjDifficulty = overallDifficulty;
  } else {
    adjDifficulty = difficulty + (overallDifficulty - minTarget) / diffRange;
  }
  ...
  if (isNaN(adjDifficulty)) {
    console.error("[difficulty] adjDifficulty is NaN!", {
      ...
      maxTarget,
      diffRange,
      calculation: `${difficulty} + ((${overallDifficulty} - ${minTarget}) / ${diffRange})`,
    });
  }
...
export function generatePhrase(settings: GenerationSettings = {}): {
  ...
  const best = allAttempts.reduce((prev, curr) =>
    Math.abs(curr.adjDifficulty - bandMid) <
    Math.abs(prev.adjDifficulty - bandMid)
      ? curr
      : prev,
  );
